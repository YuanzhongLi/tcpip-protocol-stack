...
int main(int argc, char* argv[]) {
    ...
    int sockfd; //local socket descripter
    ...
    memset(&servSockAddr, 0, sizeof(servSockAddr)); // 接続先（サーバ）側の情報が格納される
    ...
    if (connect(sockfd, (struct sockaddr*) &servSockAddr, sizeof(servSockAddr)) < 0) {
    ...
    while (totalBytesRcvd < MAX_MSGSIZE) {
        if ((byteRcvd = recv(sockfd, recvBuffer, MSGSIZE, 0)) > 0) {
